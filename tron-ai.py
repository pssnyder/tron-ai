...
class DQNAgent:
    ...
    def encode_state(self, obs, player_num):
        ...
        Features (Total: 135):
        - Local grid view (11x11 centered on player): 121 features
        - Distance to walls (top, bottom, left, right): 4 features
        - Distance to opponent (vertical, horizontal): 2 features
        - Current direction (one-hot encoded): 4 features
        - Available space estimate (safety in each direction): 4 features
        ...
        Returns:
            Encoded state vector of shape (1, 135)
        """
        grid = obs['grid']
...
if __name__ == "__main__":
    ...
    # Create agents
    dqn_agent = DQNAgent(
        state_size=150,  # Adjust based on encode_state output
        hidden_sizes=[128, 64],
        learning_rate=0.001,
        gamma=0.95,
        epsilon=1.0,
    ...
    eval_agent = DQNAgent(state_size=150)
